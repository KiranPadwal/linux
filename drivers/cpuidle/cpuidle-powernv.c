...
static int powernv_add_idle_states(void)
{
	...
	if (cpu_has_feature(CPU_FTR_STOP_INST)) {
		psscr_val = kzalloc(sizeof(*psscr_val) * dt_idle_states, GFP_KERNEL);
	...
	for (i = 0; i < dt_idle_states; i++) {
		...
		if (flags[i] & OPAL_PM_NAP_ENABLED) {
			...
			strncpy(powernv_states[nr_idle_states].name, (char *)names[i],
						MAX_IDLE_STATE_NAME_LEN);
			strncpy(powernv_states[nr_idle_states].desc, (char *)names[i],
						MAX_IDLE_STATE_NAME_LEN);
			powernv_states[nr_idle_states].flags = 0;

			powernv_states[nr_idle_states].enter = &stop_loop;
			stop_psscr_table[nr_idle_states] = (unsigned long) psscr_val[i];
		}
